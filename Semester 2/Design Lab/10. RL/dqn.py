...
class CleaningRobotEnv:
    def __init__(self, size=10, num_obstacles=5, visualize=False, delay=0.01):
        ...
        # For large grids, use sparse representation (not used in DQN version)
        ...
        # Initialize pygame for small grids
...
class DQN(nn.Module):
    def __init__(self, input_dim=4, hidden_dim=64, output_dim=5):
...
# DQN Training Loop
# -------------------------------
def run_dqn(env, episodes, lr, gamma, epsilon, max_steps, 
            replay_buffer_size, batch_size, update_freq):
    input_dim = 4
    output_dim = env.action_space.n
    policy_net = DQN(input_dim, hidden_dim=64, output_dim=output_dim).to(device)
    target_net = DQN(input_dim, hidden_dim=64, output_dim=output_dim).to(device)
    ...
    for ep in range(episodes):
        ...
        for t in range(max_steps):
            ...
            next_state_np, reward, done, _ = env.step_env(action)
            episode_reward += reward
            next_state = torch.FloatTensor(next_state_np).unsqueeze(0).to(device)
            replay_buffer.push(state, action, reward, next_state, done)
            state = next_state

            # Visualization if enabled (for small grids)
            if env.visualize and env.size <= 100:
                env.render()

            # Sample a batch and update network if enough samples exist
            if len(replay_buffer) >= batch_size:
                ...
                done_batch = torch.FloatTensor([float(b[4]) for b in batch]).to(device)
        ...
        if (ep+1) % 100 == 0:
            print(f"Episode {ep+1}, Average Reward (last 100): {np.mean(all_rewards[-100:])}")
    return policy_net, all_rewards

# -------------------------------
# Main Section with argparse
# -------------------------------
if __name__ == "__main__":
    ...
    # Default (hardcoded) parameters for DQN training when no flag is provided:
    default_params = {
        'grid_size': 10,
        'lr': 0.001,
        'gamma': 0.99,
    ...
    if args.hyperparameter:
        print("\nStarting hyperparameter tuning for DQN...")
        # Hyperparameter config for tuning
        config = {
            'grid_sizes': [10],
            'learning_rates': [0.1],
        ...
        for grid in config['grid_sizes']:
            ...
            best_config = None
            for lr, gamma, eps, steps, bs, upd in product(config['learning_rates'],
                                                           config['discount_factors'],
                                                           config['epsilons'],
                                                           config['max_steps_values'],
                                                           config['batch_sizes'],
                                                           config['update_freqs']):
                print(f"Tuning params: grid_size={grid}, lr={lr}, gamma={gamma}, epsilon={eps}, max_steps={steps}, batch_size={bs}, update_freq={upd}")
                env = CleaningRobotEnv(size=grid, num_obstacles=max(5, grid // 10), visualize=args.visualize)
                _, rewards = run_dqn(env, episodes, lr, gamma, eps, steps, replay_buffer_size=1e7, batch_size=bs, update_freq=upd)
                ...
                if final_avg_reward > best_final_reward:
                    ...
                    best_config = {'grid_size': grid, 'lr': lr, 'gamma': gamma, 'epsilon': eps,
                                   'max_steps': steps, 'batch_size': bs, 'update_freq': upd}
            tuned_params[grid] = best_config
            print(f"Best hyperparameters for grid size {grid}: {best_config}")
    
    # Evaluation Phase
    if args.evaluate:
        print("\nStarting evaluation for DQN...")
        ...
        for grid in eval_sizes:
            ...
            env = CleaningRobotEnv(size=grid, num_obstacles=max(5, grid // 10), visualize=args.visualize)
            _, rewards = run_dqn(env, episodes,
                                 params['lr'],
                                 params['gamma'],
                                 params['epsilon'],
                                 params['max_steps'],
        ...
        # Default training on grid size 10.
        if args.hyperparameter:
