...
public:
    ...
    {
        cout << findRange(root, x, y) << endl;
    }

    bool isSubTreeInRange(node *root, int x, int y)
    {
        if (!root)
            return true;
        if (root->data < x or root->data > y)
            return false;
        return isSubTreeInRange(root->left, x, y) and isSubTreeInRange(root->right, x, y);
    }
...
private:
    ...
    int findRange(node *root, int x, int y)
    {
        if (!root)
            return 0;
        int n = 0;
        if (isSubTreeInRange(root, x, y))
            return 1 + findRange(root->left, x, y) + findRange(root->right, x, y);
        return findRange(root->left, x, y) + findRange(root->right, x, y);
    }
